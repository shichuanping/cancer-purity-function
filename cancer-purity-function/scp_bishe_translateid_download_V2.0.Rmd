...
cluster_process = function(data_step3, one_cancer){
  ...
  if (class_gap > 0.1 & all(class_capacity > 5)){
    ...
    data_step4_filter_out <-c(data_step4_filter_out,fileNames_v[j])
...
feature_values = function(data_step4,one_cancer){
  ...
  one_sample_data_final = data.frame()
  for(i in 1:length(data_step4)){
    ...
    data_step4[[i]][which(data_step4[[i]]$Segment_Mean<= -0.6), 3]<- 1
    data_step4[[i]][intersect(which(data_step4[[i]]$Segment_Mean<= -0.1),which(data_step4[[i]]$Segment_Mean> -0.6)),3]<- 2
    data_step4[[i]][intersect(which(data_step4[[i]]$Segment_Mean<=  0.4),which(data_step4[[i]]$Segment_Mean> -0.1)),3]<- 3
    data_step4[[i]][intersect(which(data_step4[[i]]$Segment_Mean<= 0.9),which(data_step4[[i]]$Segment_Mean> 0.4)),3]<- 4
    data_step4[[i]][intersect(which(data_step4[[i]]$Segment_Mean<= 1.4),which(data_step4[[i]]$Segment_Mean> 0.9 )),3]<- 5
    data_step4[[i]][intersect(which(data_step4[[i]]$Segment_Mean<= 1.9),which(data_step4[[i]]$Segment_Mean> 1.4 )),3]<- 6
    data_step4[[i]][intersect(which(data_step4[[i]]$Segment_Mean<= 2.3),which(data_step4[[i]]$Segment_Mean> 1.9 )),3]<- 7
    data_step4[[i]][which(data_step4[[i]]$Segment_Mean> 2.3 ),3]<- 8
    ...
    data_result$label<-as.numeric(data_result$label)
    #Building a linear model
    myfit<-lm(data_result$Segment_Mean~data_result$label)
    #add flank value
    data_result = data.frame(label=c(1,2,3,4,5,6,7,8))
    one_sample_data = predict(myfit,data_result)
    data_result_1<-merge.data.frame(data_result,add_lenth,by.y = "label",by.x = "label",all.x = TRUE)
    data_result_1[is.na(data_result_1)] <- 0
    one_sample_data_final<-c(one_sample_data,t(data_result_1[,2]))
    one_sample_data_final<-as.data.frame(t(one_sample_data_final))
    colnames(one_sample_data_final)<-c("A","B","C","D","E","F","G","H","A1","B1","C1","D1","E1","F1","G1","H1")
    one_sample_data_final$sample<-strsplit(fileNames_new_v[i], ".txt")
    total_sample_data<-rbind(total_sample_data, one_sample_data_final)
}
    path<-paste("/home/shicp/program/download/",one_cancer,"_list0.csv",sep='')
  ...
  writepath<-paste("/home/shicp/program/final/", one_cancer, ".csv",sep='')
...
combine_sample = function(){
  ...
  #fature selecting
  all_sample_feature = feature_values(data_step4 = data_step4,one_cancer = j)
...
svm_traing = function(all_cancer_sample_feature ){
  ...
  dat1<- training_data[,c("X","A1", "B1","C1","D1","E1","F1","G1""H1","gap")]
